...
- Use `VITE_AUTH_MODE=production` for backend-owned Google and Apple auth.
...
- uses backend Google/Apple auth and refresh cookies in production mode
...
- Production login starts at `GET /api/auth/google/start` or `GET /api/auth/apple/start` and restores through `POST /api/auth/refresh`
