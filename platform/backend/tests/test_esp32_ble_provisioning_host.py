...
def test_esp32_ble_wifi_networks_payload_host_test(tmp_path):
    repo_root = Path(__file__).resolve().parents[3]
    test_src = repo_root / "device/esp32/tests_host/test_ble_wifi_networks_payload.cpp"
    payload_src = repo_root / "device/esp32/src/provisioning/wifi_networks_payload.cpp"
    arduino_json = (
        repo_root
        / "device/esp32/.pio/libdeps/esp32-s3-devkitc-1/ArduinoJson/src"
    )
    ...
    assert test_src.exists(), f"missing host test source: {test_src}"
    assert payload_src.exists(), f"missing Wi-Fi networks payload source: {payload_src}"
    assert (arduino_json / "ArduinoJson.h").exists(), (
        "missing ArduinoJson dependency; run "
        "`cd device/esp32 && pio run -e esp32-s3-devkitc-1` first"
    )
    ...
    clang = shutil.which("clang++")
    assert clang, "clang++ is required to compile the ESP32 BLE Wi-Fi networks host test"

    sdk_path = subprocess.check_output(["xcrun", "--show-sdk-path"], text=True).strip()
    output_binary = tmp_path / "test_ble_wifi_networks_payload"
    compile_cmd = [
        clang,
        "-std=c++17",
        "-isysroot",
