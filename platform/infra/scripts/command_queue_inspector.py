...
def main() -> None:
    parser = build_common_parser("Inspect recent command activity for one local PlantLab device.")
    parser.add_argument("--device-id", required=True, type=int, help="Device id to inspect.")
    parser.add_argument("--limit", default=10, type=int, help="Number of recent commands to return.")
    ...
    payload = request_json(
        "GET",
        f"{base_url}/api/devices/{args.device_id}/commands?limit={args.limit}",
        token=token,
    )
    print_json(payload)
